...
class MainButtonsWindow
{
    ...
    MainButtonsWindow(Image & WindowImage, Image ABImages[3])
    {
        WindowSprite.SetImage(WindowImage);

        AddFrameButton = AddButton(ABImages, "Add Frame", Vector2f(70, 65));

        PlayButton = AddButton(ABImages, "Play", Vector2f(70, 125));
        StopButton = AddButton(ABImages, "Stop", Vector2f(70, 125));

        LoadButton = AddButton(ABImages, "Load", Vector2f(180, 65));
        SaveButton = AddButton(ABImages, "Save", Vector2f(180, 125));

        CenterCurrentButton = AddButton(ABImages, "Center Current", Vector2f(70, 185));
        CenterAllButton = AddButton(ABImages, "Center All", Vector2f(180, 185));
    }
...
class FrameListItem
{
    ...
    FrameListItem(int FID, float Dur, Image Images[4], Vector2f Pos, Vector2f * LP, Vector2f * VP)
    {
    ...
    bool Update(Application & App, bool isSelected)
    {
        bool ReturnValue;
        UpdateDuration = false;
        ...
        if((Cursor.Left.Pressed && !TimeBox.Clicked()) || !TimeBox.Write())
        {
            UpdateDuration = true;
...
class FrameList
{
    ...
    void Update(Application & App)
    {
        ...
        {
            if(Items[i]->Update(App, (i == isSelected && Selected)))
            {
                Selected = true;
                isSelected = i;
                NewSelected = true;
...
int main()
{
    Application App(1400, 900, 32, "Skeleton Editor");
    ...
    Image WindowImage[2];
    WindowImage[0].LoadFromFile("Data/Image/EditorInterface/NewWindow01.png");
    WindowImage[1].LoadFromFile("Data/Image/EditorInterface/NewWindow02.png");

    FrameList FL;
    FL.SetPosition(ProgramSettings.Window01Position);

    MainButtonsWindow MainWindow(WindowImage[1], ABImages);
    ...
    while(App().IsOpened())
    {
        ...
        while(App().GetEvent(e))
        {
            ...
            switch(e.Key.Code)
            {
                ...
                case Key::F11:
                    ...
                    skeleton.LoadFrame("Skeleton.txt");
        ...
        if(IsPlaying)
        {
            ...
            skeleton.Draw(App, DrawBones, DrawAllLimbs);
...
//                    }
            ...
            {
                skeleton.Frames[FL.UpdateDurationFrame]->Time = FL.Items[FL.UpdateDurationFrame]->Duration;
            }
            ...
            if(SLWindow.Active && SLWindow.GetFileName() != "" && (SLWindow.SaveLoadButtonPressed || GUI::EnterKey.Pressed))
            {
                if(SLWindow.SaveLoadMode == LOAD)
                {
                    ...
                    skeleton.LoadAnimation(SLWindow.GetFileName());
...
//                    Selected = FL.Items.GetSize()-1;
                ...
                {
                    skeleton.SaveAnimation(SLWindow.GetFileName());
                }
