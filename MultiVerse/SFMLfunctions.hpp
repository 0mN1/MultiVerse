...
bool SimpleCollision(Vector2f v, Sprite s)
{
    return (v.x > s.GetPosition().x - s.GetCenter().x && v.x < s.GetPosition().x + s.GetSize().x  - s.GetCenter().x
        &&  v.y > s.GetPosition().y - s.GetCenter().y && v.y < s.GetPosition().y + s.GetSize().y  - s.GetCenter().y);
}
...
{
    point.x -= s.GetPosition().x - s.GetCenter().x*s.GetScale().x;
    point.y -= s.GetPosition().y - s.GetCenter().y*s.GetScale().y;

    if(point.x >= 0 && point.x < s.GetSize().x
    && point.y >= 0 && point.y < s.GetSize().y
    && s.GetPixel((int)(point.x/s.GetScale().x), (int)(point.y/s.GetScale().y)).a > AlphaLimit)
    return true;
    else
    return false;
}

bool PPCollisionWR(Vector2f point, Sprite & s, float AlphaLimit = 20.f)
{
    point.x -= s.GetPosition().x - s.GetCenter().x*s.GetScale().x;
    point.y -= s.GetPosition().y - s.GetCenter().y*s.GetScale().y;

    Vector2f Point1 = RotatePoint(point, s.GetCenter(), s.GetRotation());

    if(Point1.x >= 0 && Point1.x < s.GetSize().x
    && Point1.y >= 0 && Point1.y < s.GetSize().y
    && s.GetPixel((int)(Point1.x/s.GetScale().x), (int)(Point1.y/s.GetScale().y)).a > AlphaLimit)
    return true;
    else
    return false;
}
...
class Application
{
    ...
    void Update(Color c = Color(0, 0, 0))
    {
        ...
        Zoom.x = ViewSize.x / (App.GetDefaultView().GetRect().Right - App.GetDefaultView().GetRect().Left);
        Zoom.y = ViewSize.y / (App.GetDefaultView().GetRect().Bottom - App.GetDefaultView().GetRect().Top);

        Cursor.ScaledPosition.x = UpperLeft(App.GetDefaultView()).x + (Cursor.Position.x-UpperLeft(App.GetDefaultView()).x) * Zoom.x;
        Cursor.ScaledPosition.y = UpperLeft(App.GetDefaultView()).y + (Cursor.Position.y-UpperLeft(App.GetDefaultView()).y) * Zoom.y;
