...
namespace GUI
{
    ...
    SpaceKey(Key::Space),LShiftKey(Key::LShift),RShiftKey(Key::RShift),LCtrlKey(Key::LControl),RCtrlKey(Key::RControl),TabKey(Key::Tab),
    PeriodKey(Key::Period),CommaKey(Key::Comma),AddKey(Key::Equal),SubtractKey(Key::Subtract),BackKey(Key::Back),EnterKey(Key::Return),
    ...
    class TextBox
    {
        ...
        bool Clicked(Application & App)
        {
            if(Position.x < Cursor.Position.x && Cursor.Position.x < Position.x + Size.x/App.GetZoom().x
            && Position.y < Cursor.Position.y && Cursor.Position.y < Position.y + Size.y/App.GetZoom().y)
            {
                for(int i = 0; i < 4; i++)
                Rect.SetPointOutlineColor(i, Color(128, 128, 255));
        ...
        void Draw(Application & App)
        {
            ...
            Position = ViewPosition + Vector2f(Origin.x / App.GetZoom().x, Origin.y / App.GetZoom().y);

            Rect.SetPosition(ViewPosition);
            Str.SetPosition(Position.x, Position.y - (Size.y / 10)/App.GetZoom().y);

            Rect.SetScale(Vector2f(1.f / App.GetZoom().x, 1.f / App.GetZoom().y));
            Str.SetScale(Vector2f(1.f / App.GetZoom().x, 1.f / App.GetZoom().y));
        ...
        void SetSpecial(bool spec)
        {
            if(Special != spec)
            {
                ...
                if(Special)
                {
                    ...
                    {
                        for(int i = 0; i < 4; i++)
                        Rect.SetPointOutlineColor(i, Color(128, 255, 255));
                        Str.SetColor(Color(0, 0, 0));
                    }
                    else
                    {
                        for(int i = 0; i < 4; i++)
                        Rect.SetPointOutlineColor(i, Color(128, 255, 128));

                        if(S1.empty())
                        {
                            Str.SetColor(Color(150, 200, 150));
                        }
                        else
                        Str.SetColor(Color(75, 150, 75));
                    }
                }
                else
                {
