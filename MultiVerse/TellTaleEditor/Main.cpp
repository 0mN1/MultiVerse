...
#include "../TellTaleSystemUpdate.hpp"
#include "List.hpp"

using LIST::List;

enum {EDITOR, ADDCHARACTER, ADDSUBTITLE};
enum {WORLD, CHARACTER, PLAYER};
...
bool Collision(Vector2f & v, String s)
{
    return (v.x < s.GetRect().Right && v.x > s.GetRect().Left
         && v.y < s.GetRect().Bottom && v.y > s.GetRect().Top);
}
...
class AddButtonsWindow : public WindowBase
{
    ...
    AddButtonsWindow(Image & WindowImage, Image ABImages[3])
    {
        ...
        Buttons.AddNode(AddButton(ABImages, "Add Character",    Vector2f( 70,  50), 14));
        Buttons.AddNode(AddButton(ABImages, "Add Subtitle",     Vector2f(180,  50), 14));
    ...
    void Update(Application & App, int Mode)
    {
        switch(Mode)
        {
            ...
            case ADDCHARACTER:
                NameText.SetText("   AddWindow: AddCharacter");
            break;

            case ADDSUBTITLE:
                NameText.SetText("   AddWindow: AddSubtitle");
            break;
...
class AddCharacterWindow : public WindowBase
{
    protected:
    ...
    AddCharacterWindow(Image & WindowImage, Image ABImages[3])
    {
        Active = false;
        WindowSprite.SetImage(WindowImage);

        NameText.SetText("   AddCharacter");
        NameText.SetSize(15);

        Position = Vector2f(1400/2-250, 900/2-175);

        Text.AddNode(GUIString("ID", Vector2f(20, 20), 16)); // IDString : gsid = 0
        Text.AddNode(GUIString("CharacterType", Vector2f(260, 20), 16)); // EditorBodyString : gsid = 1
        Text.AddNode(GUIString("EditorBody", Vector2f(20, 60), 16)); // EditorBodyString : gsid = 2
        TextBoxes.AddNode(GUI::TextBox(20, 40, 220, 16)); // IDTextBox : tbid = 0
        TextBoxes.AddNode(GUI::TextBox(260, 40, 220, 16)); // EditorBodyTextBox : tbid = 1
        TextBoxes.AddNode(GUI::TextBox(20, 80, 220, 16)); // EditorBodyTextBox : tbid = 2
        TextBoxes[0]->SetActive(true); // Activate IDTextBox
    ...
    bool Update(Application & App, TellTaleScene * Scene)
    {
        UpdateWindow(App);

        if(Cursor.Left.Pressed)
        {
            TextBoxes[0]->Clicked(App); // Press IDTextBox
            TextBoxes[1]->Clicked(App); // Press EditorBodyTextBox
            TextBoxes[2]->Clicked(App); // Press EditorBodyTextBox
        }

        TextBoxes[0]->Write(); // Write IDTextBox
        TextBoxes[1]->Write(); // Write EditorBodyTextBox
        TextBoxes[2]->Write(); // Write EditorBodyTextBox

        TextBoxes[0]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw IDTextBox
        TextBoxes[1]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw EditorBodyTextBox
        TextBoxes[2]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw EditorBodyTextBox
        Text[0]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw IDString
        Text[1]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw EditorBodyString
        Text[2]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw EditorBodyString

        if(OkButton.Update(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)))
        {
            Scene->AddCharacter(TextBoxes[0]->GetString(), StringToInt(TextBoxes[1]->GetString()), TextBoxes[2]->GetString(), ViewPosition + Vector2f(1280/2 / App.GetZoom().x, 720/2 / App.GetZoom().y));

            TextBoxes[0]->SetString("");
            TextBoxes[1]->SetString("");
            TextBoxes[2]->SetString("");
...
class AddSubtitleWindow : public WindowBase
{
    protected:
    ...
    AddSubtitleWindow(Image & WindowImage, Image ABImages[3])
    {
        Active = false;
        WindowSprite.SetImage(WindowImage);

        NameText.SetText("   AddType01Character");
        NameText.SetSize(15);

        Position = Vector2f(1400/2-250, 900/2-175);

        Text.AddNode(GUIString("Text", Vector2f(20, 20), 16)); // IDString : gsid = 0
        Text.AddNode(GUIString("Duration", Vector2f(20, 60), 16)); // EditorBodyString : gsid = 1
        TextBoxes.AddNode(GUI::TextBox(20, 40, 460, 16)); // IDTextBox : tbid = 0
        FloatBoxes.AddNode(GUI::FloatBox(20, 80, 220, 16)); // EditorBodyTextBox : tbid = 1
        TextBoxes[0]->SetActive(true); // Activate TextTextBox

        FloatBoxes[0]->SetString("0");
    ...
    bool Update(Application & App, TellTaleScene * Scene)
    {
        ...
        {
            TextBoxes[0]->Clicked(App); // Press IDTextBox
            FloatBoxes[0]->Clicked(App); // Press EditorBodyTextBox
        }

        TextBoxes[0]->Write(); // Write IDTextBox
        FloatBoxes[0]->Write(); // Write EditorBodyTextBox

        TextBoxes[0]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw IDTextBox
        FloatBoxes[0]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw EditorBodyTextBox
        Text[0]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw IDString
        Text[1]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)); // Draw EditorBodyString

        if(OkButton.Update(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)))
        {
            Scene->AddSubtitle(TextBoxes[0]->GetString(), FloatBoxes[0]->GetValue());
...
class PropertiesWindow : public WindowBase
{
    ...
    List< AddButton > CharButtons;
    List< GUI::TextBox > CharTBox;
    List< GUI::FloatBox > CharFBox;

    List< AddButton > PCharButtons;
    List< GUI::TextBox > PCharTBox;
    List< GUI::FloatBox > PCharFBox;
    ...
    PropertiesWindow(int & mode, Image & WindowImage, Image ABImages[3], Image FBBImages[3], Image OBBImages[3])
    {
        WindowSprite.SetImage(WindowImage);

        /// World
        WorldButtons.AddNode(AddButton(ABImages, "Save", Vector2f(298, 140)));
        WorldButtons.AddNode(AddButton(ABImages, "Load", Vector2f(422, 140)));
        WorldButtons.AddNode(AddButton(ABImages, "Load Level", Vector2f(422, 200), 15));
        WorldTBox.AddNode(GUI::TextBox(250, 105, 220, 16)); // 0: NameBox
        WorldTBox.AddNode(GUI::TextBox(250, 165, 220, 16)); // 1: LevelBox
        WorldTBox.AddNode(GUI::TextBox(20, 40, 12, 16)); // 2: TypeBox
        WorldTBox[0]->SetString("CityAct01");
        WorldTBox[2]->SetString("D");
        ...
        /// Character
        CharButtons.AddNode(AddButton(ABImages, "ClearPosKey", Vector2f(298, 140), 14));
        CharButtons.AddNode(AddButton(ABImages, "AddOnAnimation", Vector2f(298, 140), 14));
        CharFBox.AddNode(GUI::FloatBox(20, 40, 50, 16)); // xPos
        CharFBox.AddNode(GUI::FloatBox(80, 40, 50, 16)); // yPos
        CharFBox.AddNode(GUI::FloatBox(140, 40, 12, 16)); // Dir

        FBButton = AddButton(FBBImages, "", Vector2f(330, 74));
        OBButton = AddButton(OBBImages, "", Vector2f(305, 74));
    }

    void Update(int nSel, Application & App, Level * Lvl, TellTaleScene * Scene, FileBrowser & FB)//, ObjectBar & OB)
    {
        UpdateWindow(App);
        ...
        switch(*Mode)
        {
            case WORLD:
            {
                ...
                if(Cursor.Left.Pressed)
                {
                    ...
                    WorldTBox[2]->Clicked(App);
                }

                if(WorldButtons[0]->Update(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)))
                {
                    Scene->Save(WorldTBox[0]->GetString());
                }

                if(WorldButtons[1]->Update(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)))
                {
                    Scene->Load(WorldTBox[0]->GetString());
                    Lvl->ClearObjectLists();
                    Lvl->Load(Scene->LevelFile, App);
                    WorldTBox[1]->SetString(Scene->LevelFile);
                    WorldTBox[2]->SetString(string(&Scene->Type));
                }

                if(WorldButtons[2]->Update(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)))
                {
                    Lvl->ClearObjectLists();
                    Lvl->Load(WorldTBox[1]->GetString(), App);
                    Scene->LevelFile = WorldTBox[1]->GetString();
                }

                WorldTBox[0]->Write();
                WorldTBox[1]->Write();
                if(WorldTBox[2]->Write())
                Scene->SetType(WorldTBox[2]->GetString(), ViewPosition + Vector2f(1280/2 / App.GetZoom().x, 720/2 / App.GetZoom().y));
                ...
                WorldTBox[2]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y));
            }
            break;
            ...
            case CHARACTER:
            {
                if(prevMode != *Mode || prevSel != nSel)
                {
                    NameText.SetText("   PropertiesWindow: Character - \"" + Scene->Characters[nSel]->ID + "\"");
                    CharFBox[0]->SetValue(Scene->Characters[nSel]->Position.x);
                    CharFBox[1]->SetValue(Scene->Characters[nSel]->Position.y);
                    CharFBox[2]->SetValue(Scene->Characters[nSel]->Direction);
                }

                if(!CharFBox[0]->GetActive())
                CharFBox[0]->SetValue(Scene->Characters[nSel]->Position.x);
                if(!CharFBox[1]->GetActive())
                CharFBox[1]->SetValue(Scene->Characters[nSel]->Position.y);
                if(!CharFBox[2]->GetActive())
                CharFBox[2]->SetValue(Scene->Characters[nSel]->Direction);

                if(Scene->Characters[nSel]->IsPositionKeyFrame(Scene->Time))
                {
                    CharFBox[0]->SetSpecial(true);
                    CharFBox[1]->SetSpecial(true);
                }
                else
                {
                    CharFBox[0]->SetSpecial(false);
                    CharFBox[1]->SetSpecial(false);
                }

                if(Scene->Characters[nSel]->IsDirectionKeyFrame(Scene->Time))
                CharFBox[2]->SetSpecial(true);
                else
                CharFBox[2]->SetSpecial(false);

                if(Cursor.Left.Pressed)
                {
                    CharFBox[0]->Clicked(App);
                    CharFBox[1]->Clicked(App);
                    CharFBox[2]->Clicked(App);
                }

                if(CharButtons[0]->Update(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y)))
                {
                    Scene->Characters[nSel]->ClearPositionKeyFrame(Scene->Time);
                }

                if(CharFBox[0]->Write())
                {
                    Scene->Characters[nSel]->SetPosition(Vector2f(CharFBox[0]->GetValue(), CharFBox[1]->GetValue()), Scene->Time);
                }
                if(CharFBox[1]->Write())
                {
                    Scene->Characters[nSel]->SetPosition(Vector2f(CharFBox[0]->GetValue(), CharFBox[1]->GetValue()), Scene->Time);
                ...
                if(CharFBox[2]->Write())
                {
                    Scene->Characters[nSel]->SetDirection(CharFBox[2]->GetValue(), Scene->Time);
                }

                CharFBox[0]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y));
                CharFBox[1]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y));
                CharFBox[2]->Draw(App, ViewPosition + Vector2f(Position.x / App.GetZoom().x, Position.y / App.GetZoom().y));
            }
            break;
            ...
            case PLAYER:
            {
                if(prevMode != *Mode || prevSel != nSel)
                {
                    NameText.SetText("   PropertiesWindow: Character - Settings");
                }
                ...
                WorldTBox[1]->Write();
                if(WorldTBox[2]->Write())
                Scene->SetType(WorldTBox[2]->GetString(), ViewPosition + Vector2f(1280/2 / App.GetZoom().x, 660 / App.GetZoom().y));
...
int main()
{
    Application App(1280, 720, 32, "TellTale Editor");
    App().SetFramerateLimit(120);

    FONT::LoadFonts();

    Level * Lvl = new Level(true, NULL, NULL);
    TellTaleScene * Scene = new TellTaleScene();

    int Mode = EDITOR;
    int EditorMode = WORLD;
    ...
    ABWindow.SetPosition(Vector2f(1280 - 250, 720 - 499));

    // AddWindows
    AddCharacterWindow  aT01Window    (WindowImages[4], ABImages);
    AddSubtitleWindow   aSubWindow    (WindowImages[4], ABImages);
    AddArtifactWindow   aArtifactWindow (WindowImages[4], ABImages, OBBImages);

    PropertiesWindow PWindow(PropertiesMode, WindowImages[4], ABImages, FBBImages, OBBImages);
    PWindow.SetPosition(Vector2f(1280 - 500, 720 - 250));
    ...
    GUIString TimeLineTime("TimePos: 0\nDuration: 10", Vector2f(5, 0));
    ...
    Vector2f lastHistory;
    TellTaleCharacter * lastChanged = NULL;
    float lastTime = 0.f;
    Vector2f lastMouse;
    ...
    while(App().IsOpened())
    {
        ...
        while(App().GetEvent(e))
        {
            ...
            if(e.Type == sf::Event::MouseWheelMoved)
            {
                ...
                {
                    if((!FB.GetActive() || !FB.CursorCollision()) && !GUI::LCtrlKey())
                    {
        ...
        if(IsPlaying)
        {
            ...
            if(GUI::LCtrlKey())
            {
                ...
                {
                    if(GUI::LShiftKey())
                    Scene->Time += 2.f * App.GetFrameTime();
                    else if(GUI::SpaceKey())
                    Scene->Time += 0.5f * App.GetFrameTime();
                    else
                    Scene->Time += App.GetFrameTime();
                }
                else if(GUI::LeftKey())
                {
                    if(GUI::LShiftKey())
            ...
            if(GUI::LCtrlKey() && GUI::ZKey.Pressed && lastChanged)
            {
                Vector2f temp = lastChanged->Position;
                lastChanged->SetPosition(lastHistory, lastTime);
                lastHistory = temp;
            }
            ...
            && (!FB.GetActive() || !FB.CursorCollision()) && (!aT01Window.GetActive() || !aT01Window.CursorCollision())
            && (!aSubWindow.GetActive() || !aSubWindow.CursorCollision()) && (!aArtifactWindow.GetActive() || !aArtifactWindow.CursorCollision()))
            {
                bool Select = false;

                for(int i = 0; i < Scene->Characters.GetSize(); i++)
                {
                    if(Collision(Cursor.Position, Scene->Characters[i]->Identification))
                    {
                        //if(Select)
                        PickUp = true;
                        Select = true;

                        PickUpOffset = Cursor.Position - Scene->Characters[i]->Position;

                        if(Selected != i || EditorMode != CHARACTER)
                        {
                            Selected = i;
                            EditorMode = CHARACTER;
                            PropertiesMode = CHARACTER;

                            if(Mode != EDITOR)
                            Mode = EDITOR;
                        }

                        lastHistory = Scene->Characters[Selected]->Position;
                        lastChanged = Scene->Characters[Selected];
                        lastTime = Scene->Time;
                    }
                    else if((Selected == i && Scene->Characters[i]->skeleton.JointSelection()))
                    {
                        Select = true;
                    }
                }

                if(Scene->Type == 'S' && Collision(Cursor.Position, Scene->PlayerCharacter->Identification))
                {
                    PickUp = true;
                    Select = true;

                    PickUpOffset = Cursor.Position - Scene->PlayerCharacter->Position;

                    if(EditorMode != PLAYER)
                    {
                        EditorMode = PLAYER;
                        PropertiesMode = PLAYER;

                        if(Mode != EDITOR)
                        Mode = EDITOR;
                    }

                    lastHistory = Scene->PlayerCharacter->Position;
                    lastChanged = Scene->PlayerCharacter;
                    lastTime = Scene->Time;
                }

                if(!Select)
                {
                    if(Mode == ADDCHARACTER)
                    aT01Window.Close();
            ...
            if(!GUI::SpaceKey() && Cursor.Right.Pressed
            && !ABWindow.CursorCollision() && !PWindow.CursorCollision()// && !OB.CursorCollision() && !OB.SlotCursorCollision()
            && (!FB.GetActive() || !FB.CursorCollision()) && (!aT01Window.GetActive() || !aT01Window.CursorCollision())
            && (!aSubWindow.GetActive() || !aSubWindow.CursorCollision()) && (!aArtifactWindow.GetActive() || !aArtifactWindow.CursorCollision()))
            {
                ...
                if(!Select && Scene->Type == 'S' && Collision(Cursor.Position, Scene->PlayerCharacter->Identification))
                {
                    PickUp = true;
                    Select = true;
            ...
            if(PickUp)
            {
                if(Cursor.Left())
                {
                    ...
                    {
                        if(lastMouse != Cursor.Position)
                        Scene->Characters[Selected]->SetPosition(Cursor.Position - PickUpOffset, Scene->Time);
                    }
                    else if(Scene->Type == 'S' && EditorMode == PLAYER)
                    {
                        if(lastMouse != Cursor.Position)
                        Scene->PlayerCharacter->SetPosition(Cursor.Position - PickUpOffset, Scene->Time);
                    }
                }
                else if(Cursor.Right())
                {
                    ...
                    {
                        if(lastMouse != Cursor.Position)
                        Scene->Characters[Selected]->SetDirection((Cursor.Position.x-PickUpOffset.x > Scene->Characters[Selected]->Position.x), Scene->Time);
                    }
                    else if(Scene->Type == 'S' && EditorMode == PLAYER)
                    {
                        if(lastMouse != Cursor.Position)
                        Scene->PlayerCharacter->SetDirection((Cursor.Position.x-PickUpOffset.x > Scene->PlayerCharacter->Position.x), Scene->Time);
                    }
                }
                else
                PickUp = false;
            ...
            Scene->Update(App, Lvl);
            Scene->UpdateCharacterPositions();

            Lvl->DrawBack(App);

            Scene->DrawCharacters(App, true, (EditorMode == PLAYER), (EditorMode == CHARACTER), Selected, lastMouse);
            ...
            if(EditorMode == CHARACTER)
            {
                String & s = Scene->Characters[Selected]->Identification;
                Shape rect = Shape::Rectangle(Vector2f(), Vector2f(s.GetRect().Right-s.GetRect().Left, s.GetRect().Bottom-s.GetRect().Top), Color(0, 0, 0, 0), 2, Color(255, 255, 255, 200));
                rect.SetCenter(s.GetCenter());
            ...
            {
                String & s = Scene->PlayerCharacter->Identification;
                Shape rect = Shape::Rectangle(Vector2f(), Vector2f(s.GetRect().Right-s.GetRect().Left, s.GetRect().Bottom-s.GetRect().Top), Color(0, 0, 0, 0), 2, Color(255, 255, 255, 200));
                rect.SetCenter(s.GetCenter());
                rect.SetPosition(s.GetPosition());
                rect.SetRotation(s.GetRotation());
            ...
            Scene->DrawSubtitles(App, ViewPosition);
            ...
            switch(Mode)
            {
                ...
                case ADDCHARACTER:
                {
                    ...
                    if(aT01Window.Update(App, Scene))
                    Mode = EDITOR;
                }
                break;
                ...
                case ADDSUBTITLE:
                {
                    ...
                    if(aSubWindow.Update(App, Scene))
                    Mode = EDITOR;
                }
                break;
...
//                    }
            ...
            if(!Scene->Playing)
            {
                PWindow.Update(Selected, App, Lvl, Scene, FB);//, OB);
            ...
            TimeLineTime.Text.SetText("Duration: " + ToString(Scene->Duration) + "\nTimePos: " + ToString(Scene->Time));
            TimeLineTime.Draw(App, ViewPosition);
