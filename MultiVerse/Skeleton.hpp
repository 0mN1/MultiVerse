...
class Limb
{
    ...
    void Draw(Application & App, Vector2f pos, bool Mirror = false, bool DrawBones = false)
    {
        ...
        if(DrawBones)
        App().Draw(Shape::Line(pos+SkeletonCenter->Vector, pos+SkeletonPoint->Vector, 2, Color(0, 0, 0)));
    }
};

enum {RARM, LARM, BODY, RLEG, LLEG, ALL};
enum {CHARTYPE01, CORPPEOPLE};
...
class Skeleton
{
    ...
    Skeleton(int bodyType = CHARTYPE01) : bones(bodyType)
    {
        BodyType = bodyType;
        ...
        if(BodyType == CHARTYPE01)
        LoadAnimation("Data/Animation/Skeleton/CharType01/LimbLenght.anm");
        else if(BodyType == CORPPEOPLE)
        LoadPosFromImage("Data/Limbs/CorpPeopleBonesColor.png");
        CalculateDistances();
    }
    ...
    {
        sprite.SetPosition(Position+bones.vectors[i]->Vector);
    ...
    Sprite & GetJointSprite(int bg, int i = 0)
    {
        if(bg == RARM)
        {
            sprite.SetPosition(Position+rArm[i]->SkeletonPoint->Vector);
        }
        else if(bg == LARM)
        {
            sprite.SetPosition(Position+lArm[i]->SkeletonPoint->Vector);
        }
        else if(bg == BODY)
        {
            sprite.SetPosition(Position+Body[i]->SkeletonPoint->Vector);
        }
        else if(bg == RLEG)
        {
            sprite.SetPosition(Position+rLeg[i]->SkeletonPoint->Vector);
        }
        else if(bg == LLEG)
        {
            sprite.SetPosition(Position+lLeg[i]->SkeletonPoint->Vector);
        }
        else if(bg == ALL)
        {
            sprite.SetPosition(Position+Body[0]->SkeletonCenter->Vector);
        }
    ...
    bool JointSelection()
    {
        for(int j = 0; j < rArm.GetSize(); j++)
        {
            sprite.SetPosition(Position+rArm[j]->SkeletonPoint->Vector);
            if(SimpleCollision(Cursor.Position, sprite))
            return true;
        }
        for(int j = 0; j < lArm.GetSize(); j++)
        {
            sprite.SetPosition(Position+lArm[j]->SkeletonPoint->Vector);
            if(SimpleCollision(Cursor.Position, sprite))
            return true;
        }
        for(int j = 0; j < Body.GetSize(); j++)
        {
            sprite.SetPosition(Position+Body[j]->SkeletonPoint->Vector);
            if(SimpleCollision(Cursor.Position, sprite))
            return true;
        }
        for(int j = 0; j < rLeg.GetSize(); j++)
        {
            sprite.SetPosition(Position+rLeg[j]->SkeletonPoint->Vector);
            if(SimpleCollision(Cursor.Position, sprite))
            return true;
        }
        for(int j = 0; j < lLeg.GetSize(); j++)
        {
            sprite.SetPosition(Position+lLeg[j]->SkeletonPoint->Vector);
            if(SimpleCollision(Cursor.Position, sprite))
            return true;
        }
    ...
    void CreateLimbs(string File)
    {
        if(BodyType == CHARTYPE01)
        {
            lArm.AddNode(new Limb("LUpperarm", "Data/Limbs/" + File + "/UpperarmLeft00.png", Vector2f(18, 25), bones.vectors.Find("CenterLUpperarm"), bones.vectors.Find("CenterLForearm")));
            lArm.AddNode(new Limb("LForearm",  "Data/Limbs/" + File + "/ForearmLeft00.png", Vector2f(35, 46), bones.vectors.Find("CenterLForearm"),  bones.vectors.Find("CenterLHand")));

            rLeg.AddNode(new Limb("RHip",  "Data/Limbs/" + File + "/ThighRight00.png", Vector2f(36, 50), bones.vectors.Find("CenterRHip"),  bones.vectors.Find("CenterRKnee")));
            rLeg.AddNode(new Limb("RKnee", "Data/Limbs/" + File + "/KneeRight.png", Vector2f(4, 5), bones.vectors.Find("CenterRKnee"), bones.vectors.Find("CenterRFoot")));
            rLeg.AddNode(new Limb("RFoot", "Data/Limbs/" + File + "/FootRight.png", Vector2f(7, 5),  bones.vectors.Find("CenterRFoot"), bones.vectors.Find("PointRFoot")));

            lLeg.AddNode(new Limb("LHip",  "Data/Limbs/" + File + "/ThighLeft00.png", Vector2f(36, 50), bones.vectors.Find("CenterLHip"),  bones.vectors.Find("CenterLKnee")));
            lLeg.AddNode(new Limb("LKnee", "Data/Limbs/" + File + "/KneeLeft.png", Vector2f(4, 5), bones.vectors.Find("CenterLKnee"), bones.vectors.Find("CenterLFoot")));
            lLeg.AddNode(new Limb("LFoot", "Data/Limbs/" + File + "/FootLeft.png", Vector2f(7, 5),  bones.vectors.Find("CenterLFoot"), bones.vectors.Find("PointLFoot")));

            Body.AddNode(new Limb("Body", "Data/Limbs/" + File + "/Body.png", Vector2f(22, 51), bones.vectors.Find("CenterBody"), bones.vectors.Find("CenterHead")));
            Body.AddNode(new Limb("HeadBack", "Data/Limbs/" + File + "/HeadBack.png", Vector2f(33, 29), bones.vectors.Find("CenterHead"), bones.vectors.Find("PointHead")));
            Body.AddNode(new Limb("HeadFront", "Data/Limbs/" + File + "/HeadFront2.png", Vector2f(33, 29), bones.vectors.Find("CenterHead"), bones.vectors.Find("PointHead")));
            ...
            rArm.AddNode(new Limb("RUpperarm", "Data/Limbs/" + File + "/UpperarmRight00.png", Vector2f(18, 25), bones.vectors.Find("CenterRUpperarm"), bones.vectors.Find("CenterRForearm")));
            rArm.AddNode(new Limb("RForearm",  "Data/Limbs/" + File + "/ForearmRight00.png", Vector2f(35, 46), bones.vectors.Find("CenterRForearm"),  bones.vectors.Find("CenterRHand")));
        }
        else if(BodyType == CORPPEOPLE)
        {
            lArm.AddNode(new Limb("LUpperarm", "Data/Limbs/" + File + "/Upperarm.png", Vector2f(12, 14), bones.vectors.Find("CenterLUpperarm"), bones.vectors.Find("CenterLForearm")));
            lArm.AddNode(new Limb("LForearm",  "Data/Limbs/" + File + "/Forearm.png", Vector2f(12, 13), bones.vectors.Find("CenterLForearm"),  bones.vectors.Find("CenterLHand")));

            rLeg.AddNode(new Limb("RHip",  "Data/Limbs/" + File + "/Thigh.png", Vector2f(20, 25), bones.vectors.Find("CenterRHip"),  bones.vectors.Find("CenterRKnee")));
            rLeg.AddNode(new Limb("RKnee", "Data/Limbs/" + File + "/LowerLeg.png", Vector2f(9, 24), bones.vectors.Find("CenterRKnee"), bones.vectors.Find("CenterRFoot")));

            lLeg.AddNode(new Limb("LHip",  "Data/Limbs/" + File + "/Thigh.png", Vector2f(20, 25), bones.vectors.Find("CenterLHip"),  bones.vectors.Find("CenterLKnee")));
            lLeg.AddNode(new Limb("LKnee", "Data/Limbs/" + File + "/LowerLeg.png", Vector2f(9, 24), bones.vectors.Find("CenterLKnee"), bones.vectors.Find("CenterLFoot")));

            Body.AddNode(new Limb("Body", "Data/Limbs/" + File + "/Body.png", Vector2f(26, 33), bones.vectors.Find("CenterBody"), bones.vectors.Find("CenterHead")));
            Body.AddNode(new Limb("Head", "Data/Limbs/" + File + "/Head.png", Vector2f(9, 34), bones.vectors.Find("CenterHead"), bones.vectors.Find("PointHead")));

            rArm.AddNode(new Limb("RUpperarm", "Data/Limbs/" + File + "/Upperarm.png", Vector2f(12, 14), bones.vectors.Find("CenterRUpperarm"), bones.vectors.Find("CenterRForearm")));
            rArm.AddNode(new Limb("RForearm",  "Data/Limbs/" + File + "/Forearm.png", Vector2f(12, 13), bones.vectors.Find("CenterRForearm"),  bones.vectors.Find("CenterRHand")));
        }
    ...
    void AddFrame()
    {
        Frames.AddNode(new Bones(BodyType));
...
//        lLegPosition = RotatePoint(lLegPosition, Vector2f(0, 0), -Angle(bones.vectors.Find("CenterBody")->Vector, bones.vectors.Find("CenterHead")->Vector)*DPR-90);
    ...
    bool Draw(Application & App, bool mirror = false, int Priority = 1, bool DrawBones = false, bool DrawAll = true)
    {
        ...
        if(CombatMode) /// Lösning finns i -> bones.vectors[i]->Vector
        {
            float dist = Distance(Vector2f(0, 0), CombatArms);
            float angle = Angle(Vector2f(0, 0), CombatArms);
            bones.vectors.Find("CenterRForearm")->Vector = MoveInAngle(angle+5/DPR, dist * 0.5f);
            bones.vectors.Find("CenterLForearm")->Vector = MoveInAngle(angle-5/DPR, dist * 0.5f);
            bones.vectors.Find("CenterRHand")->Vector = MoveInAngle(angle+5/DPR, dist * 0.5f);
            bones.vectors.Find("CenterLHand")->Vector = MoveInAngle(angle-5/DPR, dist * 0.5f);
        }
        ...
        /// Body
        {
            ...
            {
                for(int i = Body.GetSize()-2; i >= 0; i--)
                {
                    float Dist = Distance(Body[i]->SkeletonPoint->Vector, Body[i+1]->SkeletonPoint->Vector);
                    Body[i]->SkeletonPoint->Vector += MoveInAngle(Angle(Body[i]->SkeletonPoint->Vector, Body[i+1]->SkeletonPoint->Vector), Dist-Body[i+1]->Dist);
                }
...
//            lArm[i]->SkeletonPoint->Vector += Position - PrevPosition;
        ...
        if(BodyType == CHARTYPE01 && (GUI::Numpad5Key() || DrawAll))
    ...
    bool Draw(Application & App, float Time, bool mirror = false, bool Loop = false, int Priority = 1, bool DrawBones = false, bool DrawAll = true) /// TellTaleSystem
    {
        bool Restart = false;
        Mirror = mirror;
        if(Play && Frames.GetSize() > 0)
        {
            ...
            while(TimeCounter >= Frames[Index]->Time && Index < Frames.GetSize()-1)
            {
                TimeCounter -= Frames[Index]->Time;
                Index++;
                if(Index >= Frames.GetSize()-1 && Loop)
...
//            lArm[i]->SkeletonPoint->Vector += Position - PrevPosition;
    ...
    void LoadAnimation(string fn, bool Add = false)
    {
        ifstream fin;
        fin.open(fn.c_str());
